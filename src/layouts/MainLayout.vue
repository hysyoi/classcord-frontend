<template>
  <div class="top-announcement-bar">
    {{ store.isLoading ? '載入中...' : (store.servers.length === 0 ? '尚未加入班級' : (store.activeServer?.name ?? '請選擇班級')) }}
  </div>
  <div 
    class="app-layout"
    :class="{ 'quiz-active-layout': store.activeQuiz }"
  >
    <!-- 左側獨立的伺服器列表 -->
    ...
    >
      <splitpanes v-if="!store.activeQuiz" class="main-splitpanes" @resize="handlePaneResize">
        <pane :size="21" :min-size="minSidebarPercent" :max-size="maxSidebarPercent">
    ...
    <!-- 懸浮式用戶卡片 -->
    <div class="user-card-floating" v-if="store.currentUser && !store.activeQuiz" :style="{ width: userCardWidth + 'px' }">
      <div class="user-card-avatar-info">
        <div class="user-avatar" :style="{ backgroundColor: avatarColor }">
          <img v-if="store.currentUser.avatarUrl" :src="store.currentUser.avatarUrl" class="user-avatar-img" />
        ...
        <div class="user-info">
          ...
          <div class="user-status user-status-flip">
            <div class="user-status-flip-inner">
              ...
              <div class="status-back" :title="store.currentUser.email">{{ store.currentUser.email }}</div>
            </div>
          </div>
        </div>
      </div>
...
const userInitial = computed(() => {
  return store.currentUser?.username ? store.currentUser.username.charAt(0).toUpperCase() : '?'
});

const avatarColor = computed(() => {
  return store.currentUser?.username ? store.getRandomColor(store.currentUser.username) : 'var(--brand-color)'
});
...
.main-splitpanes :deep(.splitpanes__splitter::after) {
  ...
  background-color: rgba(255, 255, 255, 0.25); /* 顯著的引導灰色線條，與 Discord 原生懸浮拖拽條效果一致 */
  border-radius: 2px;
  opacity: 0;
