<template>
  <div class="member-list">
    <div v-for="group in groups" :key="group.role" class="group-section">
      <div class="group-name">{{ group.role }} — {{ group.members.length }}</div>
      ...
      <div v-for="member in group.members" :key="member.id" class="member-item">
        <div class="avatar-wrapper">
          ...
          <div class="status-dot" :class="member.online ? 'online' : 'offline'"></div>
        </div>
...
const groups = computed(() => {
  const teacherMembers = store.serverMembers.filter(m => m.role === 'TEACHER')
  const taMembers = store.serverMembers.filter(m => m.role === 'TA')
  const studentMembers = store.serverMembers.filter(m => m.role === 'STUDENT')
...
function mapMember(m: any, defaultColor: string) {
  ...
  // 優先使用 Store 中的隨機色彩產生器，依 ID 維持色調一致，若無則採用角色預設色
  const color = store.getRandomColor ? store.getRandomColor(m.id || m.userId || name) : defaultColor
  return {
    id: m.id || m.userId || Math.random().toString(),
    name,
    initial,
    color,
