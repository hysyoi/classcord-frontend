  ...
  <div v-if="isDev" class="dev-tools-container">
    ...
    <div v-if="isOpen" class="dev-panel shadow-2xl border border-slate-700/50">
      ...
      <div class="panel-body space-y-4 text-xs">
        <!-- 狀態資訊 -->
        <div class="info-group bg-[var(--bg-darkest)] p-2.5 rounded border border-slate-800 space-y-1.5">
          <div class="flex justify-between">
            <span class="text-slate-400">登入狀態:</span>
            <span :class="authStore.isAuthenticated ? 'text-green-400 font-semibold' : 'text-red-400 font-semibold'">
          ...
          <div class="flex flex-col gap-0.5">
            ...
            <span class="text-slate-300 font-mono break-all bg-slate-950 p-1 rounded mt-1 select-all overflow-y-auto max-h-12">
        ...
        <div class="flex flex-col gap-2">
          <button @click="autofill" class="action-btn bg-indigo-600 hover:bg-indigo-500">
            📝 填寫測試帳密 (Dev Email)
          </button>
          
          <button @click="handleLogout" :disabled="!authStore.isAuthenticated" class="action-btn bg-red-600 hover:bg-red-500 disabled:opacity-50 disabled:cursor-not-allowed">
            🚪 安全登出 (清除 Token)
          </button>
          
          <button @click="simulate401" :disabled="!authStore.isAuthenticated" class="action-btn bg-amber-600 hover:bg-amber-500 disabled:opacity-50 disabled:cursor-not-allowed">
            ⚠️ 模擬 401 Token 失效
          </button>

          <button @click="toggleOutlines" class="action-btn transition" :class="showOutlines ? 'bg-pink-600 hover:bg-pink-500 text-white' : 'bg-slate-700 hover:bg-slate-600 text-slate-200'">
