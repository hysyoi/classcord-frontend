    ...
    <Card class="login-card border-none">
      ...
      <CardContent class="px-8 pb-4">
        ...
        <form @submit.prevent="handleLogin" id="login-form" novalidate>
          ...
          <Button type="submit" class="w-full mt-6 bg-indigo-600 hover:bg-indigo-500 text-white cursor-pointer transition">
            登入
          </Button>
        </form>
      </CardContent>
      ...
      <CardFooter class="px-8 pb-8 flex flex-col gap-4 items-center">
        ...
        <div class="relative flex py-1 items-center w-full max-w-[336px]">
          ...
          <span class="flex-shrink mx-4 text-[10px] text-slate-500 uppercase tracking-wider font-semibold">或者使用以下方式登入</span>
          <div class="flex-grow border-t border-slate-700/50"></div>
        </div>
        ...
        <div class="text-xs text-slate-400 mt-1">
          需要一個帳號？
          <router-link to="/register" class="text-indigo-400 hover:underline transition ml-1">
            註冊
          </router-link>
        </div>
      </CardFooter>
...
onMounted(() => {
  ...
  if (code) {
    const provider = localStorage.getItem('oauth_provider') as 'GOOGLE' | 'GITHUB' | 'DISCORD' | null
...
const handleLogin = async () => {
  ...
  try {
    ...
    if (err?.body?.errors) {
      ...
      errorMessage.value = err?.body?.message || '登入失敗，請檢查電子信箱與密碼是否正確'
    }
  }
}
...
const handleOAuthCallback = async (provider: 'GOOGLE' | 'GITHUB' | 'DISCORD', code: string) => {
  errorMessage.value = ''
