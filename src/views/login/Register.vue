    ...
    <Card class="register-card border-none">
      ...
      <CardContent class="px-8 pb-4">
        ...
        <div v-if="isSuccess" class="flex flex-col items-center gap-4 text-center py-4">
          ...
          <div class="text-slate-200 text-sm font-semibold">註冊申請已受理！</div>
          <div class="text-xs text-slate-400 leading-relaxed">
            我們已向您的電子信箱 <span class="text-slate-300 font-medium">{{ email }}</span> 寄送了帳號啟用信。<br>
        ...
        <!-- 註冊表單 -->
        <form v-else @submit.prevent="handleRegister" id="register-form" novalidate>
          <FieldSet>
            <FieldGroup>
              <Field>
                <FieldLabel for="username" class="text-slate-300">使用者名稱</FieldLabel>
                <Input
                  id="username"
                  type="text"
                  v-model="username"
                ...
                />
                <span v-if="errors.username" class="text-red-400 text-[10px] mt-1 block">{{ errors.username }}</span>
              </Field>

              <Field>
                <FieldLabel for="email" class="text-slate-300">電子信箱</FieldLabel>
                <Input
                  id="email"
                  type="email"
                  v-model="email"
                  ...
                  class="bg-[var(--bg-darkest)] text-[#dbdee1] border-slate-700 focus-visible:ring-indigo-500"
                />
                <span v-if="errors.email" class="text-red-400 text-[10px] mt-1 block">{{ errors.email }}</span>
              </Field>
              
              <Field>
                <FieldLabel for="password" class="text-slate-300">密碼</FieldLabel>
                <Input 
                  id="password" 
                  type="password" 
                  v-model="password"
                  ...
                  class="bg-[var(--bg-darkest)] text-[#dbdee1] border-slate-700 focus-visible:ring-indigo-500"
                />
                <span v-if="errors.password" class="text-red-400 text-[10px] mt-1 block">{{ errors.password }}</span>
              </Field>
            </FieldGroup>
          </FieldSet>
          <Button type="submit" :disabled="isLoading" class="w-full mt-6 bg-indigo-600 hover:bg-indigo-500 text-white cursor-pointer transition">
            {{ isLoading ? '註冊中...' : '註冊' }}
          </Button>
        </form>
      </CardContent>
      ...
      <CardFooter v-if="!isSuccess" class="px-8 pb-8 flex flex-col gap-4 items-center">
        ...
        <div class="relative flex py-1 items-center w-full max-w-[336px]">
          ...
          <span class="flex-shrink mx-4 text-[10px] text-slate-500 uppercase tracking-wider font-semibold">或者使用以下方式快速註冊</span>
          <div class="flex-grow border-t border-slate-700/50"></div>
        </div>
        ...
        <div class="flex flex-col gap-2.5 w-full max-w-[336px]">
          <!-- Google 官方品牌按鈕 -->
          <Button @click="loginWithGoogle" type="button" class="relative w-full h-10 bg-[var(--bg-darkest)] hover:bg-[var(--bg-darker)] text-white font-medium transition cursor-pointer flex items-center justify-center rounded border border-slate-700/80 shadow-md">
            <svg class="absolute left-4 h-4 w-4" aria-hidden="true" focusable="false" role="img" xmlns="http://www.w3.org/2000/svg" viewBox="0 0 24 24">
          ...
          <!-- GitHub 官方品牌按鈕 -->
          <Button @click="loginWithGithub" type="button" class="relative w-full h-10 bg-[#24292F] hover:bg-[#24292F]/90 text-white font-medium transition cursor-pointer flex items-center justify-center rounded border-none shadow-md">
            <svg class="absolute left-4 h-4 w-4" aria-hidden="true" focusable="false" role="img" xmlns="http://www.w3.org/2000/svg" viewBox="0 0 24 24" fill="currentColor">
          ...
          <!-- Discord 官方品牌按鈕 -->
          <Button @click="loginWithDiscord" type="button" class="relative w-full h-10 bg-[#5865F2] hover:bg-[#5865F2]/90 text-white font-medium transition cursor-pointer flex items-center justify-center rounded border-none shadow-md">
            <svg class="absolute left-4 h-4.5 w-4.5" aria-hidden="true" focusable="false" role="img" xmlns="http://www.w3.org/2000/svg" viewBox="0 0 127.14 96.36" fill="currentColor">
...
const handleRegister = async () => {
  ...
  try {
    ...
    if (err?.body?.errors) {
      ...
      errorMessage.value = err?.body?.message || '註冊失敗，該信箱可能已被註冊。'
    }
  } finally {
