  ...
  <div class="join-page">
    <div class="join-card">
      ...
      <div v-else class="error-state">
        ...
        <button class="btn-home" @click="router.push('/channels/@me')">返回 ClassCord 首頁</button>
      </div>
    </div>
  </div>
</template>
...
onMounted(async () => {
  ...
  try {
    ...
    if (err?.status === 409 || err?.body?.message?.includes('already a member') || err?.body?.message?.includes('已加入')) {
      state.value = 'success'
      await store.fetchServers()
      await store.selectServer(serverId)
      if (store.activeChannelId) {
      ...
      state.value = 'error'
      errorMessage.value = err?.body?.message || '您所使用的班級連結已失效，或您沒有加入此班級的權限。'
    }
  }
})
</script>
